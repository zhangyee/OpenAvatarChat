...
from handlers.tts.voxcpm.voxcpm_processor import TTSVoxCPMProcessor
import modelscope

from engine_utils.directory_info import DirectoryInfo

class TTSConfig(HandlerBaseConfigModel, BaseModel):
    model_name: str = Field(default="openbmb/VoxCPM-0.5B")
    ...
    max_length: int = Field(default=4096)  # maximum token length during generation
    cfg_value: float = Field(default=2.0)  # LM guidance on LocDiT
    inference_timesteps: int = Field(default=10)  # LocDiT inference timesteps
    normalize: bool = Field(default=True)  # enable external TN tool
    denoise: bool = Field(default=True)  # enable external Denoise tool
    retry_badcase: bool = Field(default=True)  # enable retrying mode for bad cases
    retry_badcase_max_times: int = Field(default=3)  # maximum retrying times
    retry_badcase_ratio_threshold: float = Field(default=6.0)  # length restriction for bad case detection
    streaming: bool = Field(default=True)  # use streaming generation
...
class HandlerTTS(HandlerBase, ABC):
    ...
    def load(self, engine_config: ChatEngineConfigModel, handler_config: Optional[BaseModel] = None):
        if isinstance(handler_config, TTSConfig):
            # Check if model_name is a local path (relative or absolute)
            if handler_config.model_name is not None and not os.path.exists(handler_config.model_name):
                # Not a local path, try to download from modelscope
                try:
                    modelscope.snapshot_download(handler_config.model_name)
                except Exception as e:
                    logger.warning(f"Failed to download from modelscope: {e}, will try to load directly")

            self.sample_rate = handler_config.sample_rate
            for i in range(handler_config.process_num):
                process = TTSVoxCPMProcessor(self.handler_root, handler_config,
        ...
        def consumer(task_queue_map: dict[str, deque], tts_output_queue: Queue):
            while True:
                logger.debug(f"voxcpm tts output queue status: {len(task_queue_map.keys())}, {tts_output_queue.qsize()}")
                output = None
                try:
                    ...
                    logger.debug(e)
                    continue
                logger.debug(f'voxcpm output: {output}')
                key = output['key']
    ...
    def create_context(self, session_context, handler_config=None):
        ...
        if context.dump_audio:
            dump_file_path = os.path.join(DirectoryInfo.get_project_dir(), 'temp',
                                            f"dump_avatar_audio_voxcpm_{context.session_id}_{time.localtime().tm_hour}_{time.localtime().tm_min}.pcm")
    ...
    def start_context(self, session_context, context: HandlerContext):
        ...
        def task_consumer(task_inner_queue: deque, callback: callable):
            while True:
                ...
                if task is None:
                    break
                logger.debug(f'get voxcpm task audio {len(task_inner_queue), task.result_queue.qsize()}')
                try:
        ...
        if not text_end:
            ...
            if len(sentences) > 1:  # 至少有一个完整句子
                ...
                for sentence in complete_sentences:
                    ...
                    tts_info = {
                        "text": sentence,  # 不强制添加句号，保持原始标点
                        "key": task.id,
                        "session_id": context.session_id,
                        "streaming": True  # Use streaming for incremental generation
                    }
    ...
    def destroy_context(self, context: HandlerContext):
        context = cast(TTSContext, context)
        logger.info('destroy voxcpm context')
